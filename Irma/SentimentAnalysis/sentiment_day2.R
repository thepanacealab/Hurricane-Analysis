...
allTweetsDay2<-dbGetQuery(conn,
 ...
 tweettext
FROM tweets_info 
WHERE tweetcreated BETWEEN '2017-09-05' AND '2017-09-06';
")
...
Sentimentscores_day2<-cbind("day"=rep(c(2),10), "sentiment"=rownames(Sentimentscores_day2),Sentimentscores_day2)
...
ggplot(data=Sentimentscores_day2,aes(x=sentiment,y=Score, group=1))+ geom_line()+
  ...
  xlab("Sentiments")+ylab("scores")+ggtitle("Sentiments of people on Sep 5th, 2017")
