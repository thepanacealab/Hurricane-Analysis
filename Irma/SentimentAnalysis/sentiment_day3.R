...
allTweetsDay3<-dbGetQuery(conn,
 ...
 tweettext
FROM tweets_info 
WHERE tweetcreated BETWEEN '2017-09-06' AND '2017-09-07';
")
...
Sentimentscores_day3<-cbind("day"=rep(c(3),10), "sentiment"=rownames(Sentimentscores_day3),Sentimentscores_day3)
...
ggplot(data=Sentimentscores_day3,aes(x=sentiment,y=Score, group=1))+ geom_line()+
  ...
  xlab("Sentiments")+ylab("scores")+ggtitle("Sentiments of people on Sep 6th, 2017")
