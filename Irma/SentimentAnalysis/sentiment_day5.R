...
allTweetsDay5<-dbGetQuery(conn,
 ...
 tweettext
FROM tweets_info 
WHERE tweetcreated BETWEEN '2017-09-08' AND '2017-09-09';
")
...
Sentimentscores_day5<-cbind("day"=rep(c(5),10), "sentiment"=rownames(Sentimentscores_day5),Sentimentscores_day5)
...
ggplot(data=Sentimentscores_day5,aes(x=sentiment,y=Score, group=1))+ geom_line()+
  ...
  xlab("Sentiments")+ylab("scores")+ggtitle("Sentiments of people on Sep 8th, 2017")
