...
allTweetsDay1<-dbGetQuery(conn,
 ...
 tweettext
FROM tweets_info 
WHERE tweetcreated BETWEEN '2017-09-04' AND '2017-09-05';
")
...
Sentimentscores_day1<-cbind("day"=rep(c(1),10), "sentiment"=rownames(Sentimentscores_day1),Sentimentscores_day1)
...
ggplot(data=Sentimentscores_day1,aes(x=sentiment,y=Score, group=1))+ geom_line()+
  ...
  xlab("Sentiments")+ylab("scores")+ggtitle("Sentiments of people on Sep 4th, 2017")
