...
allTweetsDay9<-dbGetQuery(conn,
 ...
 tweettext
FROM tweets_info 
WHERE tweetcreated BETWEEN '2017-09-12' AND '2017-09-13';
")
...
Sentimentscores_day9<-cbind("day"=rep(c(9),10), "sentiment"=rownames(Sentimentscores_day9),Sentimentscores_day9)
...
ggplot(data=Sentimentscores_day9,aes(x=sentiment,y=Score, group=1))+ geom_line()+
  ...
  xlab("Sentiments")+ylab("scores")+ggtitle("Sentiments of people on Sep 12th, 2017")
