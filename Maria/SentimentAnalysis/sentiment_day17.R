...
allTweetsDay17<-dbGetQuery(conn,
 ...
 tweettext
FROM tweets_info 
WHERE tweetcreated BETWEEN '2017-10-02' AND '2017-10-03';
")
...
Sentimentscores_day17<-cbind("day"=rep(c(32),10), "sentiment"=rownames(Sentimentscores_day17),Sentimentscores_day17)
rownames(Sentimentscores_day17)<-NULL

saveRDS(Sentimentscores_day17, "day17.Rds")

#plotting the sentiments with scores
ggplot(data=Sentimentscores_day17,aes(x=sentiment,y=Score, group=1))+ geom_line()+
  geom_point()+
  theme(legend.position="none")+
  xlab("Sentiments")+ylab("scores")+ggtitle("Sentiments of people on Oct 2nd, 2017")
