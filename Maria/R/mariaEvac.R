...
dat <- read.csv(file="maria_evacuation_status.csv", header=TRUE, sep=",")
...
#Select Puerto Rico(72), North Carolina(37)and Virgin Island(78) subset
us.map <- us.map[us.map$STATEFP %in% c("72", "37", "78"),]
...
#Define pallete: Yellow = Emergency, Orange = Voluntary, Red = Mandatory, White = None
pal <- colorFactor(c('yellow', 'red', 'orange', white),
                            domain = leafmap$status)

#Print evacuation status per county in PR, NC and VI
