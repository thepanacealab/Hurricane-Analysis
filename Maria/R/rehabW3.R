...
allTweetsDuringW3<-dbGetQuery(conn,
"SELECT 
...
AND tweetcreated BETWEEN '2017-10-17' AND '2017-10-23'
GROUP BY  tweetuser,date(tweetcreated),lat,long,tweetpname
ORDER BY date(tweetcreated);
")

residentTweetsW3<- allTweetsDuringW3[allTweetsDuringW3$tweetuser %in% residentList,]

##GeoCode to County map
for(i in 1:nrow(residentTweetsW3)) {
  Lat <- as.double(residentTweetsW3[i,"lat"]) 
  Lon <- as.double(residentTweetsW3[i,"long"])
  #make a data frame
  ...
  residentTweetsW3[i, "countyFIPS"] <- as.character(over(points, us.map)$GEOID)
}
...
residentTweetsW3 <- residentTweetsW3[!(is.na(residentTweetsW3$countyFIPS) | residentTweetsW3$countyFIPS==""), ]

##No. of tweets per county
tweetCountPerCountyW3 <- residentTweetsW3 %>%
  group_by(tweetuser,countyFIPS) %>%
  summarize(n())
tweetCountPerCountyW3 <- aggregate(tweetCountPerCountyW3$`n()`, by=list(GEOID=tweetCountPerCountyW3$countyFIPS), FUN=sum)
